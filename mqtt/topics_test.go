...
func TestSubscribe(t *testing.T) {
	...
	for _, tx := range tt {
		t.Run(tx.desc, func(t *testing.T) {
			isNew, _ := index.Subscribe(tx.client, tx.subscription)
			require.Equal(t, tx.wasNew, isNew)
		})
	}
...
func TestUnsubscribe(t *testing.T) {
	...
	ok, _ := index.Unsubscribe("a/b/c/d", "cl1")
	require.True(t, ok)
	...
	ok, _ = index.Unsubscribe("d/e/f", "cl1")
	require.True(t, ok)
	...
	ok, _ = index.Unsubscribe("fdasfdas/dfsfads/sa", "nobody")
	require.False(t, ok)
}
...
func TestUnsubscribeNoCascade(t *testing.T) {
	...
	ok, count := index.Unsubscribe("a/b/c/e/e", "cl1")
	require.True(t, ok)
...
func TestUnsubscribeShared(t *testing.T) {
	...
	ok, count := index.Unsubscribe("$SHARE/tmp/a/b/c", "cl1")
	require.True(t, ok)
...
func TestInlineSubscribe(t *testing.T) {
	...
	for _, tx := range tt {
		t.Run(tx.desc, func(t *testing.T) {
			exist, count := index.InlineSubscribe(tx.subscription)
			require.Equal(t, tx.wasNew, exist)
...
func TestInlineUnsubscribe(t *testing.T) {
	...
	ok, count := index.InlineUnsubscribe(1, "a/b/c/d")
	require.Equal(t, 0, count)
	require.True(t, ok)
	require.Nil(t, index.root.particles.get("a").particles.get("b").particles.get("c"))
	...
	ok, _ = index.InlineUnsubscribe(1, "d/e/f")
	require.Equal(t, 0, count)
	require.True(t, ok)
	require.NotNil(t, index.root.particles.get("d").particles.get("e").particles.get("f"))

	ok, _ = index.InlineUnsubscribe(1, "not/exist")
	require.Equal(t, 0, count)
	require.False(t, ok)
}
