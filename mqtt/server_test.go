...
func TestPublishToSubscribersSelfNoLocal(t *testing.T) {
	...
	cl, r, w := newTestClient()
	s.Clients.Add(cl)
	subbed, _ := s.Topics.Subscribe(cl.ID, packets.Subscription{Filter: "a/b/c", NoLocal: true})
...
func TestPublishToSubscribers(t *testing.T) {
	...
	s.Clients.Add(cl2)
	s.Clients.Add(cl3)
	subbed, count := s.Topics.Subscribe(cl.ID, packets.Subscription{Filter: "a/b/c"})
	require.True(t, subbed)
	require.Equal(t, 1, count)
	subbed, count = s.Topics.Subscribe(cl2.ID, packets.Subscription{Filter: SharePrefix + "/tmp/a/b/c"})
	require.True(t, subbed)
	require.Equal(t, 1, count)
	subbed, count = s.Topics.Subscribe(cl3.ID, packets.Subscription{Filter: SharePrefix + "/tmp/a/b/c"})
	require.True(t, subbed)
...
func TestPublishToSubscribersMessageExpiryDelta(t *testing.T) {
	...
	cl.Properties.ProtocolVersion = 5
	s.Clients.Add(cl)
	subbed, _ := s.Topics.Subscribe(cl.ID, packets.Subscription{Filter: "a/b/c"})
	require.True(t, subbed)
...
func TestPublishToSubscribersIdentifiers(t *testing.T) {
	...
	cl.Properties.ProtocolVersion = 5
	s.Clients.Add(cl)
	subbed, _ := s.Topics.Subscribe(cl.ID, packets.Subscription{Filter: "a/b/+", Identifier: 2})
	require.True(t, subbed)
	subbed, _ = s.Topics.Subscribe(cl.ID, packets.Subscription{Filter: "a/#", Identifier: 3})
	require.True(t, subbed)
	subbed, _ = s.Topics.Subscribe(cl.ID, packets.Subscription{Filter: "d/e/f", Identifier: 4})
	require.True(t, subbed)
...
func TestPublishToSubscribersPkIgnore(t *testing.T) {
	...
	cl, r, w := newTestClient()
	s.Clients.Add(cl)
	subbed, _ := s.Topics.Subscribe(cl.ID, packets.Subscription{Filter: "#", Identifier: 1})
	require.True(t, subbed)
...
func TestProcessPublishWithTopicAlias(t *testing.T) {
	...
	cl, r, w := newTestClient()
	s.Clients.Add(cl)
	subbed, count := s.Topics.Subscribe(cl.ID, packets.Subscription{Filter: "a/b/c", Qos: 0})
	require.True(t, subbed)
	require.Equal(t, 1, count)
...
func TestPublishToSubscribersExhaustedSendQuota(t *testing.T) {
	...
	subbed, count := s.Topics.Subscribe(cl.ID, packets.Subscription{Filter: "a/b/c", Qos: 2})
	require.True(t, subbed)
	require.Equal(t, 1, count)
...
func TestPublishRetainedToClientIsShared(t *testing.T) {
	...
	subbed, count := s.Topics.Subscribe(cl.ID, sub)
	require.True(t, subbed)
	require.Equal(t, 1, count)
...
func TestServerSendLWTDelayed(t *testing.T) {
	...
	cl2.ID = "cl2"
	s.Clients.Add(cl2)
	subbed, count := s.Topics.Subscribe(cl2.ID, packets.Subscription{Filter: "a/b/c"})
	require.True(t, subbed)
	require.Equal(t, 1, count)
